...
配布対象はGitHub Releaseに`manifest.json`とONNXファイルを同梱します。本番workflowにはRelease tag、ファイル名、両ファイルのSHA-256を入力します。workflowは入力ハッシュ、manifest内のモデルハッシュ、ライセンス表記を検証し、モデル本体を先に、manifestを最後にR2へ配置します。`UNPUBLISHED`のままではデプロイできません。
